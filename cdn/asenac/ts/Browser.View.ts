//@ts-ignore
if(typeof process!="undefined") esprima = require('esprima')

namespace Asenac{


    export const PagesText:{[key:string]:string} ={};
    export const PagesJson:{[key:string]:JsonAsenac.JsonAsena[]} = {};
    export const PagesFunction:{[key:string]:Function} = {}

    /**
     * Load Asenac View Engine
     */
    export function Load(script?:string,name?:string,type:"asenac/realdata"|"asenac/function"="asenac/function"){
        // Load All 
        if(script==null){
            ...
            for (let i = 0; i < scripts.length; i++) {
                ...
                PagesText[name] = code;
                PagesJson[name] = Asenac.GetJsonAsenac(code);
            ...
            PagesText[name] = script;
            PagesJson[name] = Asenac.GetJsonAsenac(script);

            if(type=="asenac/realdata"){
                
            }
            if(type=="asenac/function"){
                PagesFunction[name] = BrowserView.complie(script);
            }
    ...
    export function Complie(pageName:string,...parameters:any){
    ...
    export function ComplieBrowserFunction(pageName:string,...parameters:any){
        return Asenac.PagesFunction[pageName].bind(this)(...parameters);
    }
    export function ComplieBrowserRealData(pageName:string,...parameters:any){
        return BrowserViewRealData.complie.bind(this)(pageName,...parameters);
    }



    export namespace BrowserView{

            
        export function complie(asena:Array<Asenac.JsonAsenac.JsonAsena>|string):Function {
            let fn =null;
         ...
         */
        export function complieFunctionString(asena:Array<Asenac.JsonAsenac.JsonAsena>|string,page?:string):string|{error:{page:string,line:string,code:string,message:string}}{
            if( typeof(asena) == "string" ){
                asena = Asenac.GetJsonAsenac(asena);
            }
            var fnStr = "("+ CreateFunctionMain(asena) +")";
         ...
         */
        function CreateFunctionMainTry(asena: Array<Asenac.JsonAsenac.JsonAsena>,name?:string ):string {
         ...
         * CreateFunctionMain
         */
        export function CreateFunctionMain(asena: Array<Asenac.JsonAsenac.JsonAsena> ):string {
            let code="function(Self,Global){\n"
            code +="if(Global==void 0) Global=Self;";
            code +="let ___that=this;\n"
            code +="let Keys={};\n"
         ...
         */
        function CreateFunctionChildren(asena: Array<Asenac.JsonAsenac.JsonAsena> ):string {
            let code="function(Self,Global){\n"
            code +="if(Global==void 0) Global=Self;";
            code +="let Parent = document.createDocumentFragment();\n"
            code +="Asenac.Docs.push(Parent);\n"
         ...
         * CreateFunction
         */
        function CreateFunction(asena: Array<Asenac.JsonAsenac.JsonAsena> ):string {
            let code = "";
        ...
        function Text(asena:Asenac.JsonAsenac._TextJson) {
            let code = ""
            code += "let Element = document.createTextNode("+EvalOuter(asena.text)+");\n"
            return code
        }
    
        function Crel(asena:Asenac.JsonAsenac._CrelJson) {
            let code = "";
         ...
         * If
         */
        function If(asena:Asenac.JsonAsenac._IfAllJson):string {
            let code = ""
         ...
         * For
         */
        function For(asena:Asenac.JsonAsenac.JsonAsena) :string {
            let code="{\n";
         ...
         * Replace
         */
        function Replace(asena:Asenac.JsonAsenac.JsonAsena): string {
            let code="{\n";
         ...
         * Append
         */
        function Append(asena:Asenac.JsonAsenac.JsonAsena): string {
            let code="{\n";
         ...
         * Append
         */
        function SetSelector(asena:Asenac.JsonAsenac.JsonAsena): string {
            let code="{\n";
        ...
        function Goto(asena:Asenac.JsonAsenac.JsonAsena):string {
            let code="";
        ...
        function Html(asena:Asenac.JsonAsenac._HtmlJson):string {
            let code = "{\n";
         ...
         * Eval
         */
        function Eval(asena:Asenac.JsonAsenac.JsonAsena):string {
         ...
         * EvalOuter
         */
        function EvalOuter(script:string):string {
            // {{ previusly }}
            var mat = null;
            // @$$$var
         ...
         */
        function EvalInner(script:string):string {
            // {{ previusly }}
            var mat = null;
            while( (mat=script.matchRequirsive("{{{","}}}")) ){
                script = script.replace(mat[0] , EvalInner(mat[1]) );
        ...
        function EvalChange(reg:RegExp,script:string,prev:string,next:string,split:RegExp=null,join:string=""):string{
            var mat=null;
            while( (mat = script.match(reg) ) != null ){
                if( mat.length>1 ){
                    var list = mat[1] as string;
                    if(split!=null)
